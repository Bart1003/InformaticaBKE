...
def Spelen():
    ...
    while True:
        play_agent = input(
            "Vul de naam in van de agent waar tegen jij wilt spelen \n")
        try:
            my_agent = load(play_agent)
        except:
            print(
                "Deze bestaat niet, selecteer alsjeblieft een uit de lijst van bestaande agenten"
            )
            continue
        else:
            my_agent = load(play_agent)
...
def Grafiek():
    ...
    if symbol == "O":
        validation_result = validate(agent_o=my_agent,
                                     agent_x=validation_agent,
                                     iterations=100)
...
def Vergelijking():
    ...
    while True:
        try:
            itteraties = int(
                input(
                    "Hoeveel iteraties tussen de 1 en 50 wil je gebruiken? \n")
            )
        except:
...
while True:
    print(
        "Maak uw selectie uit de volgende nummers:\n(1) Speel BKE tegen een vriend \n(2) Train een agent \n(3) Speel BKE tegen een agent \n(4) Grafiek van de prestaties van een agent \n(5) Vergelijking van een AI"
    )
